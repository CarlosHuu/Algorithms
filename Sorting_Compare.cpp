...
int main()
{
	...
	cin >> arr_size;
        ...
        cout << "Selection Sort with size " << arr_size << " took " << duration_selection.count() << " microseconds." << endl;
        ...
        cout << "Bubble Sort with size " << arr_size << " took " << duration_bubble.count() << " microseconds." << endl;
        ...
        cout << "Quick Sort with size " << arr_size << " took " << duration_quick.count() << " microseconds." << endl ;  
        ...
        cout << "Heap Sort with size " << arr_size << " took " << duration_heap.count() << " microseconds." << endl << endl;
...
int partition(int arr[], int low, int high) 
{
    //*迴圈可以直接到j==high，這樣最後就不用再swap，此外也不一定每次都需要i與j的交換*
...
{
    if (low < high) {
        int pi = partition(arr, low, high);  // 分割 arr[]
        quick_Sort(arr, low, pi-1);  // 對分割後的左半部分排序
        quick_Sort(arr, pi+1, high);  // 對分割後的右半部分排序
    }
}
